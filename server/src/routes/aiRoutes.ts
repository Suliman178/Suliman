...
const aiSchema = z.object({ projectId: z.string().optional(), instruction: z.string().min(1).max(4000), model: z.string().default('gpt-4o-mini'), agentRole: z.enum(['auto','planner','coder','reviewer','fixer','uiux']).default('auto') });
async function handle(req: any, res: any, mode: AgentRole) {
  ...
  const result = await aiRouter.run({ instruction: body.instruction, agentRole: mode === 'auto' ? body.agentRole : mode, model: body.model, projectName: project.name, files });
  ...
  res.json({ project, files: savedFiles, message: result.message, provider: result.provider, model: result.model, missingApiKey: !process.env.OPENAI_API_KEY });
}
